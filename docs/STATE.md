# Project state
_Last updated: 2026-09-06 by low-tier model in PR #13_

## Where we are
Phase: 1 — Monorepo scaffolding and CI (in progress)
Milestone target: M1
Current task: T-1.5 — Typed IPC bridge
Blocked: none
...
## Commands
install: pnpm install · dev: pnpm dev (opens the Electron shell since T-1.4) · build: pnpm build (since T-1.4) · typecheck: pnpm typecheck (green since T-1.2; covers all workspace packages since T-1.3) · lint: wired by T-1.6 · test: wired by T-1.7 · check: pnpm lint && pnpm typecheck && pnpm test (full chain green at T-1.9)
